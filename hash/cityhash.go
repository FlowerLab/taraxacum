...
func hash32Len13to24(s []byte, length uint32) uint32 {
	return fmix(mur(fetch32(s[length-4:]), mur(fetch32(s),
		mur(fetch32(s[(length>>1):]), mur(fetch32(s[length-8:]),
			mur(fetch32(s[4:]), mur(fetch32(s[(length>>1)-4:]),
				length)))))))
}
...
func CityHash64(s []byte, length uint32) uint64 {
	...
	v := weakHashLen32WithSeeds_3(s[length-64:], uint64(length), z)
	w := weakHashLen32WithSeeds_3(s[length-32:], y+k1, x)
	x = x*k1 + fetch64(s)
...
func CityHash128WithSeed(s []byte, length uint32, seed Uint128) Uint128 {
	...
	for tailDone = 0; tailDone < length; {
		...
		v = weakHashLen32WithSeeds_3(t[origLength-tailDone:], v.Lower64()+z, v.Higher64())
		v.setLower64(v.Lower64() * k0)
	}
...
func CityHash128(s []byte, length uint32) Uint128 {
	if length >= 16 {
		return CityHash128WithSeed(s[16:length], length-16, Uint128{fetch64(s), fetch64(s[8:length]) + k0})
	}
